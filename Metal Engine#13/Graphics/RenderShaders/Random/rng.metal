...
#include "rng_header.metal"

unsigned Rng::TausStep(const unsigned z, const int s1, const int s2, const int s3, const unsigned M)
{
    unsigned b=(((z << s1) ^ z) >> s2);
    return (((z & M) << s3) ^ b);
}

thread Rng::Rng(const unsigned seed1, const unsigned seed2, const unsigned seed3) {
